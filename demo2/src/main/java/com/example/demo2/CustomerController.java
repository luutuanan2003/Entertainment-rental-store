...
public class CustomerController {
    ...
    public void setMainApplicationCUSTOMER(MainApplicationCustomer mainApplication) {
    ...
    public String returned() {
        ...
        if (validCustomerID) {
            ...
            if (!customerExists) {
                ...
                List<String> rentedItems = getRentedItems(customerID);

                if (rentedItems.isEmpty()) {
                    displayAlert("No rented items found for Customer ID: " + customerID);
                } else {
                    // Prompt user for item ID
                    String itemID = itemIDField.getText().trim();
                    boolean validItemID = rentedItems.contains(itemID);

                    if (validItemID) {
                        displayReturnOrder(customerID, itemID);

                        // Save the return order details to the returnorders.txt file
                        saveReturnOrderDetails(customerID, itemID);
                    } else {
    ...
    private void displayReturnOrder(String customerID, String itemID) {
        // Retrieve information about the item from the items.txt file
        String itemInfo = getItemInformation(itemID);

        // Create a new window to display the return order
        Stage stage = new Stage();
        stage.setTitle("Return Order");

        // Create a VBox to hold the content of the window
        VBox vbox = new VBox();
        vbox.setSpacing(10);
        vbox.setPadding(new Insets(20));

        // Create labels to display the customer ID, item ID, and item information
        Label customerIDLabel = new Label("Customer ID: " + customerID);
        Label itemIDLabel = new Label("Item ID: " + itemID);
        Label itemInfoLabel = new Label("Item Information:\n" + itemInfo);

        // Add the labels to the VBox
        vbox.getChildren().addAll(customerIDLabel, itemIDLabel, itemInfoLabel);
    ...
    private String getItemInformation(String itemID) {
    ...
    private void saveReturnOrderDetails(String customerID, String itemID) {
        String orderDetails = customerID + "," + itemID;
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("src/main/returnorders.txt", true))) {
    ...
    private List<String> getRentedItems(String customerID) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("src/main/customers.txt"))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (customerInfo.length >= 8 && customerInfo[0].equals(customerID)) {
                    for (int i = 8; i < customerInfo.length; i++) {
                        rentedItems.add(customerInfo[i].trim()); // Trim the item ID
                    }
                    break; // Assuming the customer ID is unique, exit the loop after finding a match
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private int retrieveReturnTotal(String customerID) {
        try (BufferedReader br = new BufferedReader(new FileReader("src/main/managementCus.txt"))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] customerInfo = line.split(",");
                if (customerInfo[0].equals(customerID)) {
    ...
    private String getAccountType(String customerID) {
        try {
            File file = new File("src/main/managementCus.txt");
            BufferedReader reader = new BufferedReader(new FileReader(file));
    ...
    public void addTotalReturn(String customerID) {
        try {
            File inputFile = new File("src/main/managementCus.txt");
            File tempFile = new File("src/main/tempManagementCus.txt");
    ...
    public void guest_promoteManagement(String customerID) {
        ...
        if (customerFound) {
            ...
            try (BufferedWriter bw = new BufferedWriter(new FileWriter("src/main/managementCus.txt"))) {
    ...
    private void saveOrderDetails(String customerID, String itemID) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("src/main/orders.txt", true))) {
